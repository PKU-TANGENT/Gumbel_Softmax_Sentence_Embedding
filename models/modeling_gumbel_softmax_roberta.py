...
class GumbelSoftmaxRobertaModel(RobertaModel):
    def __init__(self, config, add_pooling_layer=False):
        super(RobertaModel, self).__init__(config)
        self.config = config

        self.embeddings = RobertaEmbeddings(config)
        self.encoder = GumbelSoftmaxRobertaEncoder(config)
    ...
    #     pass
class GumbelSoftmaxRobertaEncoder(RobertaEncoder):
    def __init__(self, config):
        super(RobertaEncoder, self).__init__()
        self.config = config
        self.layer = nn.ModuleList([GumbelSoftmaxRobertaLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
class GumbelSoftmaxRobertaLayer(RobertaLayer):
    def __init__(self, config):
        super(RobertaLayer, self).__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = GumbelSoftmaxRobertaAttention(config)
...
class GumbelSoftmaxRobertaAttention(RobertaAttention):
    def __init__(self, config, position_embedding_type=None):
        super(RobertaAttention, self).__init__()
        self.self = GumbelSoftmaxRobertaSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = RobertaSelfOutput(config)
        self.pruned_heads = set()
class GumbelSoftmaxRobertaSelfAttention(RobertaSelfAttention):
    def forward(
        self,
