...
class STSBenchmarkEval(STSEval):
    ...
    def run(self, params, batcher):
        results = {}
        train_and_dev_scores = []
        train_and_dev_gs_scores = []
        all_sys_scores = []
        all_gs_scores = []
        for dataset in self.datasets:
            sys_scores = []
            ...
            if dataset in ["train", "dev"]:
                train_and_dev_scores.extend(sys_scores)
                train_and_dev_gs_scores.extend(gs_scores)
            all_sys_scores.extend(sys_scores)
            all_gs_scores.extend(gs_scores)
            results[dataset] = {'pearson': pearsonr(sys_scores, gs_scores),
                                'spearman': spearmanr(sys_scores, gs_scores),
        ...
        all_pearson = pearsonr(all_sys_scores, all_gs_scores)
        all_spearman = spearmanr(all_sys_scores, all_gs_scores)
        train_and_dev_pearson = pearsonr(train_and_dev_scores, train_and_dev_gs_scores)
        train_and_dev_spearman = spearmanr(train_and_dev_scores, train_and_dev_gs_scores)
        results['all'] = {'pearson': {'all': all_pearson[0],
                                      'mean': avg_pearson,
                                      'wmean': wavg_pearson},
                          'spearman': {'all': all_spearman[0],
                                       'mean': avg_spearman,
                                       'wmean': wavg_spearman}}
        results["train_and_dev"] = { "pearson" : {"all": train_and_dev_pearson[0]},
                                     "spearman" : {"all": train_and_dev_spearman[0]}
        }
        logging.debug('ALL : Pearson = %.4f, \
            Spearman = %.4f' % (all_pearson[0], all_spearman[0]))
        logging.debug('ALL (weighted average) : Pearson = %.4f, \
...
class SICKRelatednessEval(STSBenchmarkEval):
    def __init__(self, task_path, seed=1111):
