...
from simcse.models.cocolm.modeling_cocolm import COCOLMModel
from simcse.models.cocolm.configuration_cocolm import COCOLMConfig
from simcse.models.cocolm.tokenization_cocolm import COCOLMTokenizer
from transformers.trainer_utils import is_main_process
logger = logging.getLogger(__name__)
MODEL_CONFIG_CLASSES = list(MODEL_FOR_MASKED_LM_MAPPING.keys())
MODEL_TYPES = tuple(conf.model_type for conf in MODEL_CONFIG_CLASSES)
@hydra.main(config_path="configs", config_name="cocolm")
def main(cfg):
    ...
    if not os.path.isdir(data_args.cache_dir):
        os.makedirs(data_args.cache_dir)
    if not data_args.force_remap and os.path.isdir(data_args.cache_file) and training_args.do_train:
        train_dataset = load_from_disk(data_args.cache_file)
    elif (data_args.force_remap or not os.path.isdir(data_args.cache_file)) and training_args.do_train:
        ...
        map_args={}
        for tmp_dict in data_args.map_args:
            for k in tmp_dict:
                map_args[k] = eval(str(tmp_dict[k]))
        train_dataset = datasets.map(
            prepare_features,
            **map_args
        )
        train_dataset.save_to_disk(data_args.cache_file)
